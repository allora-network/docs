...
For a minimal working cluster, one needs to have one head node and one worker node. For more detailed information about setting up identities and everything, refer to [this Readme](blockless-network-general-concepts) and [the node documentation](blockless-node-b7s-usage).
