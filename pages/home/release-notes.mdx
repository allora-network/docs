...
- **Per-topic c_norm**: c_norm is now a configurable per-topic field instead of a global parameter.
...
- **OOIF matrix**: Fix matrix for events with value bundle.
- **Active topic set**: Fix potential active topic set issue.
