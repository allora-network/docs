...
[Forge competitions](/build/forge/competitions).
...
- **`unknown service emissions.v9.QueryService`** in the worker log — the
  ...
  upgrade (see [Networks](/reference/networks)). Upgrade it inside the kit's
...
- **Deploy more topics** — `TOPIC_ID=77 python deploy_worker.py` deploys the
  same artifact to another topic; browse [existing topics](/build/forge/topics)
  and discover them programmatically with `AlloraTopicDiscovery`.
- **Understand the competition** — [how Forge competitions
  work](/build/forge/competitions), from testnet track record to mainnet ALLO
  rewards.
- **Monitor on-chain** — [query worker data](/build/worker/query-worker-data)
  and [worker monitoring](/build/worker/monitoring) cover scores and EMA
