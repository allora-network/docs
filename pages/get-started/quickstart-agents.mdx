...
- **Coming:** `https://docs.allora.network/llms-full.txt` — the full docs
  corpus in a single file. Not yet live; do not assume it exists.
- **Coming:** a raw-markdown version of each docs page. Until it lands, fetch
...
A `SANITY CHECK WARNING` in the log is expected: the placeholder `123.45` is
far from the topic's consensus BTC/USD price. It does not block submission on
the sandbox topic.
...
- **Faucet fails or rate-limits** (`Too many faucet requests`) — request
  funds directly, authenticated with your API key, for the wallet address
  printed in the worker's startup banner:
