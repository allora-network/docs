...
- `allorad` -  Used to read and write data to the chain, e.g. to create a wallet, create new topics or add/delegate stake to a reputer
  - Refer to the [Allorad Reference](/reference/allorad) section for a full list of `allorad` commands with their explanations
...
The command below installs {CHAIN_VERSION_TESTNET}, the version currently deployed on the testnet. If you are targeting mainnet, pass {CHAIN_VERSION_MAINNET} instead — see [Networks](/reference/networks) for the version deployed on each network.
