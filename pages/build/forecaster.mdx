...
A **forecaster** is a worker that predicts how accurate other workers' inferences will be, instead of (or in addition to) answering the topic's question itself. For each inferer, it submits a **forecasted loss** — an estimate of the error that inferer's inference will show against the eventual ground truth. The network turns forecasted losses into [regrets and weights](/home/layers/forecast-synthesis/forecast) and combines them into [forecast-implied inferences](/home/layers/forecast-synthesis/synthesis#forecast-implied-inferences), making the network inference [context-aware](/home/layers/forecast-synthesis/forecast#context-awareness) — better than any individual model's output. [Forecast and Synthesis](/home/layers/forecast-synthesis) explains the mechanism end to end.

<Callout type="warning">
**Standalone forecaster tooling is not published yet.** The high-level [`AlloraWorker`](/devs/sdk/allora-sdk-py) in the current Python SDK release (`allora_sdk` 1.0.6) automates the **inferer** role only; first-class forecaster support is in development on the SDK's public [dev branch](https://github.com/allora-network/allora-sdk-py/tree/dev) for an upcoming release. If you want to participate on Allora today, the smoothest path is running an inference worker — see the [Python SDK guide](/devs/sdk/allora-sdk-py) or the [price prediction worker walkthrough](/devs/workers/walkthroughs/walkthrough-price-prediction-worker).
...
- Python 3.10–3.13
- A wallet mnemonic whose `allo...` address holds a small amount of ALLO for gas. On testnet, request funds from [faucet.testnet.allora.network](https://faucet.testnet.allora.network). If you already ran an [inference worker](/devs/sdk/allora-sdk-py), you can reuse the mnemonic it saved to its `.allora_key` file.
- A topic with active inferers to forecast on. The example uses [Allora's testnet sandbox topic (ID 69)](https://testnet.explorer.allora.network/topics/69); browse [existing topics](/devs/get-started/existing-topics) for others.
...
Both fields are strings: the inferer's `allo...` address and your forecasted loss for its inference in the current epoch. Because `inference_value` is required, a forecaster built on the published SDK always infers *and* forecasts — a combination the network explicitly supports ([some workers do both](/home/layers/forecast-synthesis/forecast#losses)).
...
- **`Topic ... has no network inferences yet`** — forecasting needs inferences to forecast against. Pick a topic with active inferers ([existing topics](/devs/get-started/existing-topics)), or wait until the topic completes an epoch with inference submissions.
...
- How forecasts become weights and forecast-implied inferences: [Forecast](/home/layers/forecast-synthesis/forecast) and [Synthesis](/home/layers/forecast-synthesis/synthesis)
- Submit plain inferences with the high-level worker: [Allora Python SDK](/devs/sdk/allora-sdk-py) or the [price prediction worker walkthrough](/devs/workers/walkthroughs/walkthrough-price-prediction-worker)
- Find topics with active inferers: [existing topics](/devs/get-started/existing-topics)
- Inspect your worker's on-chain data: [query worker data using allorad](/devs/workers/query-worker-data)
