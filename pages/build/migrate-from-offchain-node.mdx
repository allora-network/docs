...
<Callout type="info">
The published SDK release (`allora_sdk` <Version of="allora-sdk"/>) also covers reputers: `AlloraWorker.reputer(...)` takes a reputer function built from your ground-truth and loss functions via `make_reputer_function(get_ground_truth, loss_fn)`, and `min_stake_uallo` replaces the offchain node's `minStake` — see [Build a Reputer](/build/reputer/build-a-reputer). The offchain node also continues to run reputers.
</Callout>
...
| `reputer[]` | `AlloraWorker.reputer(...)` — `groundTruthEntrypointName` + `lossFunctionEntrypointName` become Python functions combined with `make_reputer_function(get_ground_truth, loss_fn)`, and `minStake` becomes `min_stake_uallo` — see [Build a Reputer](/build/reputer/build-a-reputer) |
...
```python file=../../snippets/migrate_worker.py
```
...
- **You run a reputer** — migrate it to `AlloraWorker.reputer(...)` ([Build a Reputer](/build/reputer/build-a-reputer)), or keep your `reputer` configuration on the offchain node — it still runs reputers.
