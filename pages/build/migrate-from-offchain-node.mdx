...
The published SDK release (`allora_sdk` 1.0.6) supports **workers only**. The offchain node's `reputer` configuration (`groundTruthEntrypointName`, `lossFunctionEntrypointName`, `minStake`, ...) has no SDK equivalent yet — [reputers](/devs/reputers/reputers) continue to run on the offchain node for now.
</Callout>
...
| `wallet.submitTx` | Not needed — the worker submits transactions; use the [RPC client](/devs/sdk/allora-sdk-py#allorarpcclient-typed-chain-queries-and-transactions) directly for query-only use |
...
If you served multiple topics from one `config.json` `worker` array, run one script per topic with its own `topic_id`. The [Python SDK page](/devs/sdk/allora-sdk-py) covers the full set of options (`fee_tier`, `polling_interval`, `debug`, custom `AlloraNetworkConfig`).
...
- See a full worker build, from data to deployment: [price prediction worker walkthrough](/devs/workers/walkthroughs/walkthrough-price-prediction-worker)
- Compete with your model: [Model Forge quickstart](/devs/get-started/model-forge)
- Full worker, RPC, and API client reference: [Allora Python SDK](/devs/sdk/allora-sdk-py)
- Pick a topic to serve: [existing topics](/devs/get-started/existing-topics)
