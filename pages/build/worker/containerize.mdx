---
title: Build and Deploy a Worker Node using Docker
description: "Deprecated: the allora-offchain-node stack described on this page is deprecated in favor of running a worker with the Allora Python SDK and the Forge Builder Kit."
persona: ML builder
...
<Callout type="error">
**Deprecated:** the `allora-offchain-node` stack described on this page is deprecated in favor of running a worker with the [Allora Python SDK](/consume/sdk-py) and the [Forge Builder Kit](https://github.com/allora-network/allora-forge-builder-kit). For new workers — and to move an existing offchain-node worker — follow the [migration guide](/build/migrate-from-offchain-node).
</Callout>
...
1. `nodeRpc`: The [RPC URL](/get-started/setup-wallet#rpc-url-and-chain-id) for the corresponding network the node will be deployed on
2. `addressKeyName`: The name you gave your wallet key when [setting up your wallet](/get-started/setup-wallet)
3. `addressRestoreMnemonic`: The mnemonic that was outputted when setting up a new key

{/* <Callout type="info">
`addressKeyName` and `addressRestoreMnemonic` are optional parameters. If you did not previously generate keys, keys will be generated for you when [running the node](/build/worker/containerize#generate-keys-and-export-variables).
...
The `config.example.json` file that was copied and edited in the previous steps also contains a JSON object for configuring and deploying a [reputer](/build/reputer/deploy-docker). To ignore the reputer and only deploy a worker, delete the reputer sub-object from the `config.json` file.
...
<Callout>
The model in `allora-offchain-node` is barebones and outputs a random integer. For a full example that trains a real price model and serves its predictions, see the [price prediction worker walkthrough](/build/worker/sdk-py).
</Callout>
...
`allora-offchain-node` comes preconfigured with a `main.go` file inside the [`adapter/api/apiadapter` folder](https://github.com/allora-network/allora-offchain-node/blob/dev/adapter/api/apiadapter/main.go).
