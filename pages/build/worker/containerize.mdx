...
<Callout type="error">
**Deprecated:** the `allora-offchain-node` stack described on this page is deprecated in favor of running a worker with the [Allora Python SDK](/devs/sdk/allora-sdk-py) and the [Forge Builder Kit](https://github.com/allora-network/allora-forge-builder-kit). For new workers — and to move an existing offchain-node worker — follow the [migration guide](/devs/workers/migrate-from-offchain-node).
</Callout>
...
<Callout>
The model in `allora-offchain-node` is barebones and outputs a random integer. For a full example that trains a real price model and serves its predictions, see the [price prediction worker walkthrough](/devs/workers/walkthroughs/walkthrough-price-prediction-worker).
</Callout>
