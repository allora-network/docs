---
title: Model Forge Quickstart
description: The Allora Model Forge Competition is an open-source hackathon where participants attempt to create the best model for a given topic on the Allora Network.
persona: ML builder
verified_against: docs content as of 2026-07-30
last_reviewed: 2026-07-30
---

import { Callout, Code, Pre } from "nextra/components";
import { CHAIN_VERSION_TESTNET } from "../../../components/versions";

# Model Forge Quickstart

The [Allora Model Forge Competition](https://forge.allora.network) is an open-source hackathon where participants attempt to create the best model for a given [topic](/operate/topics/lifecycle) on the Allora Network.
...
Open your terminal and run the following command to install the `allorad` CLI tool ({CHAIN_VERSION_TESTNET}, the version currently deployed on the testnet).

<Pre hasCopyCode data-language="bash">
  <Code>{`curl -sSL https://raw.githubusercontent.com/allora-network/allora-chain/dev/install.sh | bash -s -- ${CHAIN_VERSION_TESTNET}`}</Code>
</Pre>
...
2. Connect your wallet to the [Allora Forge](https://forge.allora.network) site.
...
The Allora Network is categorized into distinct [topics](/operate/topics/create) that serve as Schelling points for model-makers to submit inferences on.
...
<Callout>
[Additional fields that define a given topic](/operate/topics/create) can be pulled using the following [`allorad`](/build/forge/competitions#download-the-allorad-cli-tool) command:
...
We've built the **Allora Forge Builder Kit** to help you get started with model creation: it takes you from historical data to a deployed inference worker, with feature engineering, model evaluation, and worker monitoring built in.

[**Allora Forge Builder Kit Repository**](https://github.com/allora-network/allora-forge-builder-kit)
- Features:
  - Workflow API for backfilling historical data, engineering features, and building training datasets
  - Evaluation that grades your model against Allora's scoring methodology before you deploy
  - Deployment tooling: wallet creation, faucet funding, and worker lifecycle management
  - Web monitoring dashboard with submission history, on-chain scores, and live logs

If you previously built models with the deprecated Model Development Kit (MDK), see the [migration guide](/build/migrate-from-offchain-node).
...
<Callout>
Don't forget to [connect your wallet](/build/forge/competitions#successful-registration) to the Allora Forge site to view the leaderboard.
</Callout>

## Detailed Performance Metrics

We've built out an [Allora Explorer](https://explorer.allora.network) that displays performance metrics to help you evaluate the success of your model. These metrics are used to score your model in the competition.
