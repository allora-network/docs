...
import { SandboxTopicIds } from '../../../components/TopicsTable'
...
Forge runs competitions on an ongoing basis. Each competition targets one live [topic](/build/forge/topics) on the Allora Network, carries its own ALLO prize pool, and moves through **upcoming → active → ended** as its start and end dates pass. Browse [forge.allora.network](https://forge.allora.network) for the competitions that are open right now.
...
<Callout>
No whitelist yet? The testnet **playground topics** — the [sandbox topics <SandboxTopicIds network="testnet" />](/build/forge/topics#start-here-the-sandbox-topic) — are the recommended starting point and require no whitelist, so you can build, deploy, and score a worker end to end while your registration is pending.
</Callout>
...
If you previously built models with the deprecated offchain node or Model Development Kit (MDK), see the [migration guide](/build/migrate-from-offchain-node).
...
- Pick a topic to compete on: [existing topics](/build/forge/topics)
- Deploy your first worker: [build a price prediction worker](/build/worker/sdk-py)
- Coming from the offchain node or MDK: [migrate to the Python SDK + Builder Kit](/build/migrate-from-offchain-node)
