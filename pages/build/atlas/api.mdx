...
persona: ML builder
verified_against: forge-data-service @ b381bc7; allora-forge-builder-kit 3.0.0 (main @ 8ef3200); every example executed against forge-data.allora.run (2026-07-30)
...
Query the [Atlas data platform](/build/atlas/overview) from the command line and from Python: discover datasets, fetch candles at any resolution, bulk-download history, and stream rows in real time.

## Prerequisites

- An Allora API key — self-serve at the [Allora Developer Portal](https://developer.allora.network) (keys are prefixed `UP-`; the same key works for the [Allora API](/consume/api))
...
Datasets are protected by [tags](/build/atlas/overview#tag-based-access). A fresh key holds no tags, so acquire the `public` tag first — one idempotent call:
...
Each resolution caps the time range of a single request — `raw` 24 hours, `5m` 7 days, `1h` 90 days, `1d` unlimited (see [range limits](/build/atlas/overview#resolutions-and-range-limits)). Omit `start`/`end` and the query defaults to the most recent allowed window; exceed the cap and you get:
...
`AtlasDataManager` refuses base URLs outside `forge-data.allora.run` to protect your API key from being sent elsewhere, and plugs directly into the Builder Kit's `AlloraMLWorkflow` for feature engineering and model training — see [Forge competitions](/build/forge/competitions) for that path.
...
- The concepts behind the API: [Atlas overview](/build/atlas/overview)
- Feed the data into a model: [build a price prediction worker](/build/worker/sdk-py)
- Deploy against a live topic: [Forge competitions](/build/forge/competitions)
