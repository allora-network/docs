...
To run a reputer only, configure just the `reputer` array — the `worker` section of the example config is for [inference workers](/build/worker/containerize) and can be removed.
...
`is-reputer-registered` should return `true`, and your stake should be at least the `minStake` you configured. Once the node is submitting loss bundles, track your EMA score and compare it against the lowest score in the topic's active set to confirm you are eligible for rewards — see [How to Query Reputer EMA Scores](/build/reputer/query-ema-score).
...
- **Registered and submitting, but no rewards** — verify your stake meets the chain's [`required_minimum_stake`](/reference/params/chain#required_minimum_stake); below it, your contributions are ignored. Also compare your EMA score against the topic's active set as shown in [How to Query Reputer EMA Scores](/build/reputer/query-ema-score).
...
- Understand scoring and the active set: [How to Query Reputer EMA Scores](/build/reputer/query-ema-score)
