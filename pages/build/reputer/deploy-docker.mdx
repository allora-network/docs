...
The `config.example.json` file that was copied and edited in the previous steps also contains a JSON object for configuring and deploying a [worker](/build/worker/containerize). To ignore the worker and only deploy a reputer, delete the reputer sub-object from the `config.json` file.
