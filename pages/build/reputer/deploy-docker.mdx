...
The `config.example.json` file that was copied and edited in the previous steps also contains a JSON object for the node's legacy inference-worker mode (deprecated — see [Migrate from the Offchain Node](/build/migrate-from-offchain-node)). To ignore the worker and only deploy a reputer, delete the worker sub-object from the `config.json` file.
...
For a guide based on the current `allora-offchain-node` configuration format — including the ground-truth and loss-function service contract, staking, and verification — see [Build a Reputer](/build/reputer/build-a-reputer).