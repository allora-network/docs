...
- **Mint** (`mint`): The Allora mint module account is the only account allowed to create new tokens. It creates new tokens during its `BeginBlock` according to the [Allora emission schedule](./params/mint) and then immediately sends those tokens to the Fee Collector account.
