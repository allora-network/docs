...
The `config.example.json` file that was copied and edited in the previous steps also contains a JSON object for configuring and deploying a [worker](/devs/workers/deploy-worker/using-docker). To ignore the worker and only deploy a reputer, delete the reputer sub-object from the `config.json` file.
...
For a guide based on the current `allora-offchain-node` configuration format — including the ground-truth and loss-function service contract, staking, and verification — see [Build a Reputer](/devs/reputers/build-a-reputer).