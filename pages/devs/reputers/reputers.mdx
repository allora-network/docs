...
The Flask application calls the `get_truth` function upon API request, with the required arguments (`Token`, `blockHeight`). Before proceeding, ensure that all necessary packages are listed in the `requirements.txt` file.
