...
There are workers today, mostly maintained by Upshot, that produce inferences for a set of <a href="https://docs.allora.network/docs/existing-allora-appchain-topics" target="_blank">existing topics</a>. They can be queried for inferences via standard Blockless function calls, which in turn can be used today in on-chain applications via <a href="https://docs.allora.network/docs/adapters" target="_blank">Allora adapter contracts</a>. A Blockless function call can be submitted in a format like this:
