...
There are workers today, mostly maintained by Upshot, that produce inferences for a set of [existing topics](./existing-topics). They can be queried for inferences via standard Blockless function calls, which in turn can be used today in on-chain applications via [Allora adapter contracts](./adapters). A Blockless function call can be submitted in a format like this:
