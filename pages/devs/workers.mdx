...
3. The worker node calls a node function that invoke custom logic for the actual inference.

<Callout type="info">
The request-response flow is effectively bidirectional:
- From the Allora chain to the head node.
- From the head node to the worker node.
- From the worker node to the node function.
- Finally, to the model server.
</Callout>