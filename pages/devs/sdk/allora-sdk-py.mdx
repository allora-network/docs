...
- The client defaults to testnet topics; pass `chain_id=ChainID.MAINNET` (from `allora_sdk.api_client`) to read mainnet topics.
...
- Find a topic to predict on: [existing topics](/devs/get-started/existing-topics)
- Productionize your worker: [deploy a worker with Docker](/devs/workers/deploy-worker/using-docker)
- Consume inferences from your app: [Allora API endpoint](/devs/consumers/allora-api-endpoint) or the [TypeScript SDK](/devs/sdk/allora-sdk-ts)
