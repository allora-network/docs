...
`allora-offchain-node` comes preconfigured with a `main.py` file that uses Flask to expose the Inference Endpoint. 

The Flask application calls the `get_inference`  function upon API request, with the required argument (`Token`). Before proceeding, ensure that all necessary packages are listed in the `requirements.txt` file.
