...
import { CHAIN_VERSION_TESTNET, CHAIN_VERSION_TESTNET_BARE } from '../../../../components/versions'
...
Allora Network provides convenient precompiled binaries. The version currently deployed on the testnet is {CHAIN_VERSION_TESTNET}.

### Download allorad

<Pre hasCopyCode data-language="bash">
  <Code>{`wget https://github.com/allora-network/allora-chain/releases/download/${CHAIN_VERSION_TESTNET}/allora-chain_${CHAIN_VERSION_TESTNET_BARE}_linux_amd64`}</Code>
</Pre>
...
<Pre hasCopyCode data-language="bash">
  <Code>{`mv allora-chain_${CHAIN_VERSION_TESTNET_BARE}_linux_amd64 allorad
mv allorad /usr/local/bin
chmod a+x /usr/local/bin/allorad`}</Code>
</Pre>
...
If it shows the version you just installed (<Code>{CHAIN_VERSION_TESTNET_BARE}</Code>), the installation is complete, and a `.allorad` folder will be created in the root directory.

![allorad Version Output](/alibaba/picture3.jpg)

## 4. Configure the Allora Worker Node

This guide uses the [allora-offchain-node](https://github.com/allora-network/allora-offchain-node) repository as an example.
...
The `docker-compose.yaml` in `allora-offchain-node` builds the `node` service from source. To use a prebuilt image instead, replace `build: .` in the `node` service with a pinned image from [Docker Hub](https://hub.docker.com/r/alloranetwork/allora-offchain-node/tags):

```yml
image: alloranetwork/allora-offchain-node:v0.14.1
```
...
The worker's predictions come from a separate inference server: the `source` service in `docker-compose.yaml`, built from the [`adapter/api/source` folder](https://github.com/allora-network/allora-offchain-node/tree/dev/adapter/api/source). The bundled server is a barebones example — replace its `main.py` with your own model server. The example below is a complete inference server that you can customize for your predictions.

<details>
<summary>Click to view the example inference server code</summary>
...
- [Allora Offchain Node Repository](https://github.com/allora-network/allora-offchain-node)
