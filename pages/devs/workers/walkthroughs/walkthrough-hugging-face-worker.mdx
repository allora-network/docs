...
- Familiarity with Allora Network documentation on [allocmd](/devs/get-started/installation/cli#installing-allocmd) and [building and deploying a worker node from scratch](/devs/workers/deploy-worker/build-and-deploy-worker-from-scratch).
...
First, install `allocmd` as [explained in the documentation](/devs/get-started/installation/cli#installing-allocmd).
...
Now that we know our inference server is working as expected, lets ensure it can interact with the [Blockless network](https://blockless.network/). This is how Allora nodes respond to requests for inference from chain validators.
...
- Change the accepted token or the timeframe to predict on another [Allora topic](/devs/get-started/existing-topics)
