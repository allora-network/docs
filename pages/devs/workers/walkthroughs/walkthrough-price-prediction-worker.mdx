...
- **`TIMEFRAME`**: Defines the timeframe of the data used in the format like `10m` (minutes), `1h` (hours), `1d` (days), etc.
  
  - For Coingecko, the data granularity (candle's body) is automatic. To avoid downsampling when using Coingecko:
    - Use a **`TIMEFRAME`** of `>= 30m` if **`TRAINING_DAYS`** is `<= 2`.
...
- **`MODEL`**: Specifies the machine learning model to use. Must be one of the following:
  ...
  > You can easily add support for other models by adding them to the configuration [here](#).
