...
To conveniently relay inferences from worker nodes to developers, Allora Labs has created an endpoint to solicit inferences from worker nodes and merge them into a format compatible with our consumers. The output of this endpoint can be passed directly as-is to our consumer. To use this endpoint and obtain inferences, one can use the curl requests below, depending on the topic of interest. 
...
Inferences returned by either of the above means are [Unsafe Requests](/devs/consumers#unsafe-requests).
</Callout>
