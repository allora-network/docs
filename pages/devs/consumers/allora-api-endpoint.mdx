...
Replace `<apiKey>` with your actual API key, `<chainId>` with a supported chain ID (see the `SignatureFormat` values in the [TypeScript SDK reference](/devs/sdk/allora-sdk-ts) for supported values), and `<topicId>` with the topic ID you want to query.
