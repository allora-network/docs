...
1. Call the `getTopicValue(uint256 topicId, bytes extraData)` function with the correct `topicId` on the consumer contract for your desired chain. You can provide `''` for `extraData` in solidity, or `'0x'` for `extraData` in typescript. The list of deployed contracts and topicIds can be found under [deployments](./existing-adapters).
