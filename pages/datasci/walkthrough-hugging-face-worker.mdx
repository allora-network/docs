...
```
At the time of writing, the latest `allocmd` version is `2.0.1`:
```bash
allocmd --version
allocmd version 2.0.1
```
...
We will create a very simple Flask application to serve inference from the Hugging Face model. In this example, we will be using [amazon/chronos-t5-tiny](https://huggingface.co/amazon/chronos-t5-tiny) model. 
...
# get the data
# Note: this is the key part, where you'll define the data to use to forecast the future values 
# You could change this to be data prices for a token for instance, fetching it dynamically to be updated on each call 
df = pd.read_csv("https://raw.githubusercontent.com/AileenNielsen/TimeSeriesAnalysisWithPython/master/data/AirPassengers.csv")

# define the context and the prediction length
context = torch.tensor(df["#Passengers"]) # target variable
prediction_length = 1 # only predict the next value
...
def get_inference():
    try:
        ...
        return Response(str(forecast[0].mean().item()), status=200) # returning the mean forecasted value
...
{
  "code": "200",
  "request_id": "339feec1-14ed-4f2c-979b-ac1dc8500b9f",
  "results": [
    {
      "result": {
        "stdout": "{\"infererValue\": \"438.32623291015625\"}\n\n",
        "stderr": "",
        "exit_code": 0
      },
      "peers": [
...
> You will also need to fund your worker node's address by using `allocmd fund <address>`
...
- Use your own data for your use case: price prediction, APY forecasting, gas fees predictions, ...
