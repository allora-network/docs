...
- Install the `allorad`client by following the instructions <a href="https://docs.allora.network/docs/cli-spec#install" target="_blank">here</a> 
...
- ```Text bash
  ...
  ```
- Fund the Key on the network environment you plan to run the worker, (i.e. testnet faucet, the worker will be running on testnet).  [(How to faucet your account)](how-to-faucet-your-account)
...
2. Blockless keys, which are p2p <a href="https://docs.libp2p.io/concepts/fundamentals/peers/" target="_blank">peer</a> keys. Blockless keys are used for communication with other Allora Network blockless-based nodes, including head, worker, and reputer nodes.
