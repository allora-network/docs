...
<Callout type="warning">
  Running the installation command may output `Installation complete.` even though there are **errors** during the installation process.
</Callout>
...
```bash
  ...
  bash: line 39: wget: command not found
  mv: rename /tmp/allorad to /Users/kushdave/.local/bin/allorad: No such file or directory
  chmod: /Users/kushdave/.local/bin/allorad: No such file or directory
  Installation complete. The allorad is now available in /Users/kushdave/.local/bin
  To make allorad available from any terminal session, add the following line to your .bashrc or .zshrc:
  export PATH="$PATH:/Users/kushdave/.local/bin"
```

In the example above, the message `wget: command not found` indicates that `wget` needs to be installed for `allorad` to install successfully. Despite this error, the script still logs `Installation complete.` after encountering issues such as `No such file or directory` for the `allorad` module.
