...
- **Mint** (`mint`): The Allora mint module account is the only account allowed to create new tokens. It creates new tokens during its `BeginBlock` according to the [Allora inflation schedule](https://docs.allora.network/reference/params/chain#halving_interval) and then immediately sends those token to the Fee Collector account.
