...
verified_against: allora-offchain-node (github.com/allora-network/allora-offchain-node, dev branch) config.example.json + README; allora_sdk 1.3.0 (PyPI)
...
This page runs a reputer on the [`allora-offchain-node`](https://github.com/allora-network/allora-offchain-node), which handles registration, staking, submission windows, and transaction retries. You supply two HTTP services: one that returns the ground truth and one that computes the loss. The [Python SDK](https://docs.allora.network/consume/sdk-py) can run the same role in-process — see the callout in step 5.
...
- Running an inference worker too? [Migrate a worker from the offchain node](https://docs.allora.network/build/migrate-from-offchain-node)
