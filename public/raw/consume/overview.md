...
**On-chain consumer contracts are being rebuilt.** Documentation for deploying and integrating consumer contracts will return when the new contracts ship. In the meantime, consume inferences through the [Allora API](https://docs.allora.network/consume/api) or via [RPC](https://docs.allora.network/consume/rpc-grpc).
