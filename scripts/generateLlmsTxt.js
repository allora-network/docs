...
function splitFrontmatter(content) {
  ...
  if (lines[0] !== '---') return { data: {}, body: lines };
  for (let i = 1; i < lines.length; i++) {
    if (lines[i] === '---') {
      return { data: parseFrontmatter(lines.slice(1, i)), body: lines.slice(i + 1) };
    }
  }
  return { data: {}, body: lines }; // unterminated block
}
...
// pages/ is not part of a section and is listed before the first H2, which
// llmstxt.org treats as the primary/essential set.
...
// site-absolute links become absolute URLs so the bundle stands on its own.
function mdxToMarkdown(mdxFile, bodyLines) {
...
function build() {
  ...
  // keeps llms.txt itself honest.
  const invalid = all.filter(page => !page.title || !page.description);
  if (invalid.length > 0) {
    console.error(
      `llms.txt generation failed: ${invalid.length} of ${all.length} pages are missing ` +
  ...
  return {
    index: renderIndex(resolved.rootPages, resolved.sections),
    full: renderFull(resolved.all),
    pages: all,
  };
}
